...
class GYZSelectPayMethodView: UIView {
    
    var delegate: GYZSelectPayMethodViewDelegate?
    ...
    func onClickPayView(sender: UITapGestureRecognizer){
        let tag = sender.view?.tag
        
        if tag == 101 {// 支付宝支付
            alipayCheckBtn.isSelected = true
            weChatCheckBtn.isSelected = false
            delegate?.didSelectPayType(isAliPay: true)
        } else {//微信支付
            alipayCheckBtn.isSelected = false
            weChatCheckBtn.isSelected = true
            delegate?.didSelectPayType(isAliPay: false)
        }
    }
}
protocol GYZSelectPayMethodViewDelegate:NSObjectProtocol {
