...
class PlaceAroundTableView: UIView, UITableViewDataSource, UITableViewDelegate, AMapSearchDelegate {
    ...
    var currentAddress: AMapReGeocode?
    ...
    func onReGeocodeSearchDone(_ request: AMapReGeocodeSearchRequest!, response: AMapReGeocodeSearchResponse!) {
        ...
        if response.regeocode != nil {
            self.currentAddress = response.regeocode
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        if indexPath.section == 0 {
            if self.delegate!.responds(to: #selector(PlaceAroundTableViewDeleagate.didPositionCellTapped(currAddress:))) {
                self.delegate!.didPositionCellTapped(currAddress: self.currentAddress!)
            }
        }
        else {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if indexPath.section == 0 {
            ...
            cell!.detailTextLabel?.text = self.currentAddress?.formattedAddress
